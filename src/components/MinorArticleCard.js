...
export default function MinorArticleCard({blog}) {
    ...
    return (<div>
                <Link to={`/blog/${blog.node.frontmatter.slug}`} key={blog.node.id}>
                    <li className={`${MajorArticleCardStyles.articleCard} ${MajorArticleCardStyles.secondary}`} key={blog.node.frontmatter.slug}>
                        ...
                        <p className={`${MajorArticleCardStyles.articleExcerpt} ${MajorArticleCardStyles.secondary}`}>{blog.node.excerpt}</p>
                        </div>
                    </li>
                </Link>
            </div>
